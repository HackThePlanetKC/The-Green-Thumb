...
**Not every module is BLE-paired like the above.** [`modules/`](modules/) also holds standalone modules that run their own OS/platform and speak MQTT directly rather than relaying through a base - the first is the **Camera Module** (`modules/Camera Module/`), a Pi-based visual health monitor with a WS2812B RGBW fill flash. The flash subsystem, WiFi setup, MQTT base discovery/association, grid layout, module-global/per-base settings scoping, the wilt-watch and drama-level comparison metrics, six heuristic OpenCV visual detectors (chlorosis, necrosis, spotting, leaf scorch, powdery mildew, pest indicators), a local image library (per-zone photo storage/retention, portal library pages, an opt-in downsampled thumbnail passthrough to HA over MQTT), and its own top-level MQTT/HA presence are all built; capture scheduling (when a photo actually gets taken) and its light sensor driver aren't yet - see [its README](modules/Camera%20Module/README.md).
