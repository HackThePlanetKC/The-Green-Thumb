...
- **`Pillow`** (`pip install Pillow`) - required by `image_library.py` (real JPEG storage/retention, thumbnail downsampling - see [`README.md`](README.md#image-library-local-storage-retention-portal-library-ha-thumbnail-passthrough)) and, for loading a saved reference/previous image from disk, `image_compare.py`'s `load_grayscale_from_file()`. Import-guarded in both files with a clear `RuntimeError` if missing, so the rest of the module still runs without it - only actually needed once you're exercising real image capture/storage, not just running the stub-based test suite.
